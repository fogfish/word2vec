...
namespace w2v
{
    ...
    w2vModel_t::w2vModel_t() : model_t<std::string>() {}
    ...
    bool w2vModel_t::save(const std::string &_modelFile) const noexcept
    {
        try
        {
            ...
            // file header
            std::string fileHeader = std::to_string(m_mapSize) + " " + std::to_string(m_vectorSize) + "\n";
            // calc output size
            // header size
    ...
    d2vModel_t::d2vModel_t(uint16_t _vectorSize) : model_t<std::size_t>()
    {
    ...
    bool d2vModel_t::save(const std::string &_modelFile) const noexcept
    {
        try
        {
            ...
            off_t fileSize = msSize + vsSize                                                     // header size
                             + (sizeof(std::size_t) + m_vectorSize * sizeof(float)) * m_mapSize; // record size
            // write data to the file
    ...
    void d2vModel_t::set(std::size_t _id, const vector_t &_vector, bool _checkUnique)
    {
        if (_checkUnique)
        {
            for (auto const &i : m_map)
    ...
    void d2vModel_t::erase(std::size_t _id)
    {
        m_map.erase(_id);
        m_mapSize = m_map.size();
    }
    ...
    word2vec_t::word2vec_t(const std::unique_ptr<w2vModel_t> &_model) : vector_t(_model->vectorSize()){};

    word2vec_t::word2vec_t(const std::unique_ptr<w2vModel_t> &_model, const std::string &_word) : vector_t(_model->vectorSize())
    {
